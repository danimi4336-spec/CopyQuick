...
const { listRecognizedBackups, parseRetention, resolveBackupConfig, verifySqliteBackup } = require('./databaseBackup');
...
function inspectStorageHealth({ env = process.env, db, fsApi = fs } = {}) {
  ...
  const status = quickCheck !== 'ok' || !readable || !writable
    ? 'critical'
    : capacityStatus === 'unknown' ? 'healthy' : capacityStatus;
