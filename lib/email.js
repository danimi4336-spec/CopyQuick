...
function buildAdminEmail({ ticketNumber, name, email, subject, message, ip, userAgent, submittedAt }) {
  const body = card(`
    <div style="margin-bottom:16px;">
      ...
      <span style="display:inline-block;margin-left:8px;background:#fef3c7;color:#92400e;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;padding:3px 10px;border-radius:4px;">Open</span>
    </div>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
            ...
            <tr>
              ...
              <td style="padding:4px 0;">
                <span style="display:inline-block;background:#fef3c7;color:#92400e;font-size:11px;font-weight:700;text-transform:uppercase;padding:2px 8px;border-radius:4px;">Open</span>
              </td>
            </tr>
            <tr>
...
async function sendContactFormEmails({ name, email, subject, message, ip, userAgent }) {
  ...
  const { data: adminData, error: adminError } = await resend.emails.send({
    from: 'CopyQuick Support <onboarding@resend.dev>',
