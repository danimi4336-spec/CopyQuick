...
function stateDefaults() {
  return {
    ...
    consecutiveFailureCount: 0, lastFailureAt: null,
    lastAttemptId: null, lastFailureAttemptId: null
  };
}
...
async function retrieveAndDecryptOffsiteBackup({
  objectKey,
  config,
  storage,
  encryptedPath,
  ...
  if (!recognizedObjectKey(objectKey, config.prefix)) {
    throw new OffsiteBackupError('Remote backup object is not recognized.', 'UNRECOGNIZED_REMOTE_OBJECT');
  }
  ...
  if (head.sizeBytes !== body.length || metadata.ciphertextsha256 !== parsed.header.ciphertextSha256 ||
      metadata.keyid !== parsed.header.keyId || metadata.formatversion !== String(parsed.header.version)) {
  ...
  const decrypted = decryptArtifactFile({
  ...
  return {
    ...
    artifactHash: decrypted.header.ciphertextSha256,
    plaintextSizeBytes: decrypted.sizeBytes
  };
}
...
async function createOffsiteBackup({
  ...
  try {
    ...
    writeOffsiteState(config, {
      ...previousState, lastAttemptAt: attemptAt, lastAttemptId: attemptId
    }, fsApi);
    let sourcePath;
    ...
    try {
      const failureState = readOffsiteState(config, fsApi);
      const distinctFailure = failureState.lastFailureAttemptId !== attemptId;
      writeOffsiteState(config, {
        ...failureState,
        ...
        consecutiveFailureCount: distinctFailure ? Number(failureState.consecutiveFailureCount || 0) + 1 : Number(failureState.consecutiveFailureCount || 0),
        lastFailureAt: attemptAt || now().toISOString(),
        lastAttemptId: attemptId,
        lastFailureAttemptId: attemptId
      }, fsApi);
    } catch (_) {}
...
function inspectOffsiteFreshness({ env = process.env, fsApi = fs, now = () => new Date() } = {}) {
  if (!enabled(env)) return {
    enabled: false, status: 'disabled', lastSuccessAt: null, ageHours: null,
    keyId: null, remoteRetention: null, lastAttemptAt: null, lastFailureAt: null,
    consecutiveFailureCount: 0
  };
  ...
  if (state.stateInvalid) {
    return {
      ...
      lastAttemptAt: null, lastFailureAt: null, consecutiveFailureCount: 0
    };
  }
  if (!state.lastSuccessAt) {
    return {
      enabled: true, status: 'never_succeeded', lastSuccessAt: null, ageHours: null,
      keyId: state.keyId, remoteRetention: config.retention,
      lastFailureCode: state.lastFailureCode ? normalizeOperationalCode(state.lastFailureCode) : null,
      lastAttemptAt: state.lastAttemptAt, lastFailureAt: state.lastFailureAt,
...
async function prepareOffsiteRestore({ objectKey, env = process.env, storage, decryptionKey, decryptionKeyId, fsApi = fs, now = () => new Date(), logger = entry => console.log(JSON.stringify(entry)) } = {}) {
  ...
  try {
    ...
    const retrieved = await retrieveAndDecryptOffsiteBackup({
      objectKey, config, storage: remote, encryptedPath,
      plaintextPath: plaintextTemporaryPath, decryptionKey, decryptionKeyId, fsApi
    });
    ...
    safeLog(logger, 'offsite_restore_candidate_prepared', { keyId: retrieved.header.keyId });
    return { success: true, restoreCandidatePath: finalPath, keyId: retrieved.header.keyId };
  } catch (error) {
