...
function stateDefaults() {
  return { lastAttemptAt: null, lastSuccessAt: null, lastSuccessObjectKey: null, lastFailureCode: null, keyId: null, artifactSizeBytes: null, ciphertextSha256: null };
}
...
async function createOffsiteBackup({ source, env = process.env, storage, fsApi = fs, now = () => new Date(), logger = entry => console.log(JSON.stringify(entry)) } = {}) {
  ...
  try {
    ...
    if (source) sourcePath = validateLocalSource(source, config, fsApi);
      ...
      const localResult = await createDatabaseBackup({ env, fsApi, logger });
