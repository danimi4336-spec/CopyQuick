...
function createLoginRateLimiter(options = {}) {
  ...
  function middleware(req, res, next) {
    ...
    // Valid login attempts reserve their IP and email slots before DB/bcrypt work.
    // Malformed requests do not count, success clears only the email bucket, and
    // internal errors leave the bounded attempt record until normal expiry.
    ipStore.increment(ipKey);
    emailStore.increment(emailKey);
  ...
  function recordFailure(req) {
    // Login attempts are counted before authentication to close concurrent bursts.
    return req.authLogin;
  }
