...
const { planForPrice } = require('./billingEntitlement');
...
function persistUsageTransaction(db, {
  ...
  return db.transaction(() => {
    ...
    db.prepare(`
      INSERT INTO usage_events (
        user_id, usage_period_id, generation_id, production_run_id, production_job_id,
        event_type, units, source_route, metadata
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      userId,
      usagePeriodId,
      ...
      reference.productionJobId || null,
      eventType,
      units,
      sourceRoute,
      JSON.stringify(metadata)
...
function persistUsageReversalsTransaction(db, {
  userId,
  usagePeriodId,
  productionRunId,
  jobs,
  ...
  if (!usagePeriodId) throw new Error('Missing usage period for production reversal');
  return db.transaction(() => {
    persistState(db);
    const reversalEventIds = [];
    jobs.forEach(function(job) {
      const inserted = db.prepare(`
        INSERT OR IGNORE INTO usage_events (
          user_id, usage_period_id, generation_id, production_run_id, production_job_id,
          event_type, units, source_route, metadata
        ) VALUES (?, ?, NULL, ?, ?, 'production_reversal', -1, ?, ?)
      `).run(
        userId,
        usagePeriodId,
        productionRunId,
        job.id,
        sourceRoute,
        JSON.stringify({ deliverableId: job.deliverable_id, reason: job.reversalReason })
      );
      if (inserted.changes === 1) {
        const eventId = db.prepare('SELECT last_insert_rowid() AS id').get().id;
        reversalEventIds.push(eventId);
        db.prepare(`
          UPDATE production_jobs SET reversal_usage_event_id = ?, updated_at = CURRENT_TIMESTAMP
          WHERE id = ? AND reversal_usage_event_id IS NULL
        `).run(eventId, job.id);
        recordProductionJobEvent(db, {
          productionRunId,
          productionJobId: job.id,
          eventType: 'usage_reversed',
          attemptNumber: job.attempt_count || 0,
          metadata: { reversalEventId: eventId, reasonCode: job.reversalReason }
        });
      }
    });

    const reversalCount = reversalEventIds.length;
    if (reversalCount) {
      db.prepare(`
        UPDATE usage_periods
        SET usage_count = MAX(usage_count - ?, 0), updated_at = CURRENT_TIMESTAMP
        WHERE id = ? AND user_id = ?
      `).run(reversalCount, usagePeriodId, userId);
      db.prepare(`
        UPDATE users
        SET generations_used = MAX(generations_used - ?, 0)
        WHERE id = ?
      `).run(reversalCount, userId);
      const usagePeriod = db.prepare(`
        SELECT usage_count FROM usage_periods WHERE id = ? AND user_id = ?
      `).get(usagePeriodId, userId);
      db.prepare(`
        UPDATE users SET current_period_used = ?, current_usage_period_id = ? WHERE id = ?
      `).run(usagePeriod?.usage_count || 0, usagePeriodId, userId);
    }
    return { reversalCount, reversalEventIds };
  })();
}
...
function syncSubscriptionRecord({
  ...
  db.prepare(`
    INSERT INTO subscriptions (
      ...
      latest_stripe_event_created, latest_stripe_event_id, past_due_since
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  const usagePeriod = syncUsagePeriod ? ensureUsagePeriod(db, {
    userId,
...
module.exports = {
  ...
  persistUsageReversalsTransaction,
