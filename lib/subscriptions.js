...
function persistGenerationUsageTransaction(db, {
  ...
  if (!usagePeriodId) {
    throw new Error('Missing usage period for generation usage');
  }

  return db.transaction(() => {
    const generationId = persistGeneration(db);
    ...
    return { generationId, usageCount: usagePeriod?.usage_count || 0 };
  })();
}
