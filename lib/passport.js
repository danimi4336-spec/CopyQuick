  ...
  passport.use(new GoogleStrategy({
    ...
    (accessToken, refreshToken, profile, done) => {
      ...
      try {
        ...
        console.log('  Lookup by google_id:', user ? 'found user' : 'not found');

        if (!user && email) {
          user = db.prepare('SELECT * FROM users WHERE email = ?').get(email);
          console.log('  Lookup by email:', user ? 'found user' : 'not found');
          if (user) {
        ...
        if (!user) {
          ...
          console.log('  New Google-authenticated user created.');
        }

        if (!user) {
          console.error('❌ Failed to find or create user for Google profile.');
        ...
        console.error('❌ GoogleStrategy error.');
